...
public class InstanceRestful {
  ...
  public Message getServiceInstance(
    ...
    Stream<NacosServiceInstance> instanceList =
        serviceStream
            .flatMap(serviceId -> discoveryClient.getInstances(serviceId).stream())
            .map(instance -> (NacosServiceInstance) instance);
    if (StringUtils.isNotBlank(ip)) {
      instanceList = instanceList.filter(s -> s.getHost().equals(ip));
    }
    //      if (StringUtils.isNotBlank(hostName)) {
    //        instanceList = instanceList.filter(s ->
    //        s.getInstanceInfo().getHostName().equals(hostName));
    //      }
