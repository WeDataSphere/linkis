 ...
 * 1. Check if the queue selection feature is enabled
 * 2. Check if the engine type is in the supported list
 * 3. Check if the creator is in the supported list
 * 4. Call LinkisManager RPC to get the optimal queue
 * 5. Replace the queue configuration in runtimeParams
 */
class QueueSelectionInterceptor extends EntranceInterceptor with Logging {
   ...
   *
   * @param jobRequest  The job request to process
   * @param logAppender Used to cache necessary reminder logs and pass them to the upper layer
   * @return The processed job request
   */
  override def apply(jobRequest: JobRequest, logAppender: lang.StringBuilder): JobRequest = {
    Utils.tryAndWarnMsg {
      ...
      if (!Configuration.SECONDARY_QUEUE_ENABLED.getValue) {
        logger.debug("Queue selection is not enabled at Entrance layer")
        return jobRequest
      }
      ...
      if (labels == null || labels.isEmpty) {
        logger.debug("No labels found in job request, skip queue selection")
        return jobRequest
      }
      ...
      val response = sender.ask(SecondaryYarnRequest(startMap, labels), RPC_TIMEOUT) match {
          case r: SecondaryYarnResponse =>
            logger.debug(s"Received RPC response: ${r.selectQueue}")
            r
          case other =>
      ...
      if (response == null) {
        logger.info("RPC returned null or timed out, using primary queue")
        logAppender.append("Queue selection failed: unable to get response from LinkisManager, using primary queue\n")
        return jobRequest
      }
      ...
      if (StringUtils.isNotBlank(yarnQueue)) {
        val oldQueue = startMap.get(ECConstants.YARN_QUEUE_NAME_CONFIG_KEY)
        startMap.put(ECConstants.YARN_QUEUE_NAME_CONFIG_KEY, yarnQueue)
        logger.info(s"Queue selection completed - switched from $oldQueue to $yarnQueue")
        logAppender.append(
        ...
        logger.info("Selected queue is blank, using primary queue")
        logAppender.append(s"Queue selection error - selected queue is blank, using primary queue\n")
      }
    } {
      logAppender.append(
        "Queue selection failed due to exception, using original queue configuration\n"
      )
     "Exception occurred during queue selection, using original queue"
    }
    jobRequest
  }
